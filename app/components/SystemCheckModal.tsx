...
export default function SystemCheck() {
  ...
  const modalTitle = step === 3 ? "Analyse gestartet" : "Dein Erstgespräch vorbereiten";
    ...
    >
      ...
      <div className="text-(--ink) leading-normal box-border relative z-[2] w-full max-w-155 overflow-hidden shadow-[0_40px_90px_-20px_rgba(0,0,0,0.5)] animate-[pop_0.3s_cubic-bezier(.2,0.9,0.3,1.15)] max-h-[92vh] flex flex-col m-0 p-0 rounded-md">
        ...
        <div className="text-(--ink) leading-normal box-border border-b-(--divider) relative flex-[0_0_auto] m-0 px-7.5 py-6.5 border-b border-solid bg-white">
          ...
          <div className={`${mono} leading-normal box-border text-xs tracking-[0.16em] text-(--ink) uppercase flex items-center gap-2.25 mb-2.5 m-0 p-0`}>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-3.75 h-3.75 text-(--accent)">
        ...
        <div className="text-(--ink) leading-normal box-border overflow-y-auto m-0 p-7.5 bg-(--background)">
          ...
          {step === 1 && (
            <div className="text-(--ink) leading-normal box-border block m-0 p-0">
              <label className={`${mono} leading-normal box-border text-[13px] font-semibold tracking-[0.04em] text-(--ink) uppercase block mb-3.5 m-0 p-0`}>
                ...
                {huerdeOptions.map((opt) => (
                  ...
                  >
                    ...
                    <svg
                      ...
                      stroke="currentColor"
                      strokeWidth={2}
                      className={`w-[18px] h-[18px] text-(--primary) ${huerde === opt ? "opacity-100" : "opacity-0"}`}
                    >
              ...
              <div className="text-(--ink) leading-normal box-border flex items-center gap-4 justify-end m-0 p-0">
                <button
                  type="button"
                  onClick={() => goStep(2)}
                  className="box-border inline-flex bg-(--primary) items-center gap-2.5 font-bold text-[13px] tracking-[0.08em] uppercase cursor-pointer text-white transition-[background-color,transform] duration-[250ms] m-0 px-6.5 py-3.75 border-0 hover:bg-(--violet) hover:-translate-y-px"
                >
                  Weiter
          ...
          {/* STEP 2 */}
          {step === 2 && (
            <form onSubmit={handleSubmit} className="text-(--ink) leading-normal box-border m-0 p-0">
              ...
              <div className="text-(--ink) leading-normal box-border grid grid-cols-[1fr_1fr] gap-4.5 mb-4.5 m-0 p-0">
                <div>
                  <label className={`${mono} leading-normal box-border text-xs font-semibold tracking-[0.04em] text-(--ink) uppercase block mb-2 m-0 p-0`}>
                    Ansprechpartner Name *
                  </label>
                  <input name="name" required placeholder="z.B. Martin Huber" className={inputBase} />
                </div>
                <div>
                  <label className={`${mono} leading-normal box-border text-xs font-semibold tracking-[0.04em] text-(--ink) uppercase block mb-2 m-0 p-0`}>
                    Mitarbeiteranzahl
                  </label>
                  <select name="mitarbeiter" className={inputBase} defaultValue="6 - 15 Mitarbeiter">
              ...
              <div className="grid grid-cols-[1fr_1fr] gap-4.5 mb-4.5">
                <div>
                  ...
                  <input name="telefon" required placeholder="+49 (0) 170 1234567" className={inputBase} />
                </div>
                <div>
                  <label className={`${mono} leading-normal box-border text-xs font-semibold tracking-[0.04em] text-(--ink) uppercase block mb-2 m-0 p-0`}>
                    E-Mail-Adresse *
                  </label>
                  <input type="email" name="email" required placeholder="huber@beispiel.de" className={inputBase} />
                </div>
              </div>

              <div className="mb-4.5">
                <label className={`${mono} leading-normal box-border text-xs font-semibold tracking-[0.04em] text-(--ink) uppercase block mb-2 m-0 p-0`}>
                  Bestehende Systeme (z.B. Lexware, Kuller, Excel, etc.) – optional
                </label>
                <textarea
                  name="systeme"
              ...
              <div className={`${mono} flex gap-3 bg-[rgba(46,26,71,.04)] border border-(--divider) rounded-[5px] p-4 mt-[6px] mb-6 text-[12.5px] leading-[1.6] text-(--muted)`}>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-[18px] h-[18px] flex-none text-(--accent) mt-[1px]">
                  <path d="M12 2 4 5v6c0 5 3.5 8 8 9 4.5-1 8-4 8-9V5z" />
                  <path d="m9 12 2 2 4-4" />
                </svg>
                <span>
                  Keine Werbeanrufe. Wir rufen dich ausschließlich an, um deine Systemanforderungen kurz
                  abzugleichen und dir konkrete Lösungsideen aufzuzeigen.
                </span>
              </div>

              <div className="flex items-center justify-between gap-4">
                <button type="button" onClick={() => goStep(1)} className="bg-none border-0 font-semibold text-[15px] text-(--muted) cursor-pointer hover:text-(--ink)">
                  Zurück
                </button>
                <button
                  type="submit"
          ...
          {step === 3 && (
            <div className="text-(--ink) leading-normal box-border text-center m-0 px-1.5 py-4.5">
              <div className="text-center box-border w-18.5 h-18.5 flex items-center justify-center mt-0 mb-5 mx-auto p-0 rounded-full bg-[#e9f5ec]">
                <svg viewBox="0 0 24 24" fill="none" stroke="#2e9d5b" strokeWidth={2.5} className="w-8.5 h-8.5">
                  <circle cx={12} cy={12} r={10} />
                  <path d="m8 12 3 3 5-6" />
                </svg>
              </div>
              ...
              <p className="text-[15px] text-(--muted) leading-[1.6] max-w-[52ch] mx-auto mb-6">
                Deine Angaben wurden analysiert. Ein Systemingenieur prüft deine Problemstellung
                bezüglich <b className="text-(--ink) font-bold">{huerde || "deiner Herausforderung"}</b> im
                Bereich <b className="text-(--ink) font-bold">{gewerk || "deinem Gewerk"}</b>.
              </p>
              <div className="border border-(--divider) rounded-[5px] bg-white px-[22px] py-5 text-left max-w-[460px] mx-auto mb-6.5">
                <div className={`${mono} text-xs font-semibold tracking-[0.1em] uppercase text-(--ink) border-b border-(--divider) pb-2.5 mb-3.5`}>
                  Nächste Schritte:
                </div>
                <ol className="list-none">
                  <li className={`${mono} text-[13px] leading-[1.55] text-(--ink) flex gap-2.5 mb-3 before:content-['1.'] before:font-bold before:text-(--primary)`}>
                    Wir melden uns telefonisch innerhalb von 24 Stunden bei dir.
                  </li>
                  <li className={`${mono} text-[13px] leading-[1.55] text-(--ink) flex gap-2.5 mb-3 before:content-['2.'] before:font-bold before:text-(--primary)`}>
                    Kurze Besprechung deiner Schnittstellen und Insellösungen.
                  </li>
                  <li className={`${mono} text-[13px] leading-[1.55] text-(--ink) flex gap-2.5 mb-3 before:content-['3.'] before:font-bold before:text-(--primary)`}>
              ...
              <button
                onClick={handleClose}
                className="inline-flex items-center gap-2.5 bg-[#efeae6] text-(--ink) font-bold text-[13px] tracking-[0.08em] uppercase px-6.5 py-3.75 border-0 cursor-pointer transition-colors duration-[250ms] hover:bg-(--divider)"
              >
                Fenster schließen
              </button>
            </div>
